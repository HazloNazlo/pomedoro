...
export function PostForm({ 
  ...
  const [date, setDate] = useState(initialData?.scheduled_for?.split('T')[0] || '');
  const [time, setTime] = useState(initialData?.scheduled_for?.split('T')[1]?.substring(0, 5) || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!date || !time) {
      return; // Prevent scheduling without date/time
    }
    try {
